...
- Supports both `.mjs/.cjs/.js/.ts/.mts` and `.json` mocks.
...
The mocked API consistst of a number of `.mjs/.cjs/.js/.ts/.mts` or `.json` files in a directory structure that represents the API.
...
const mock = defineMock({
    ...
    defaultResponse(request) {
        return {
            status: 200,
            body: { message: `user ${request.parameters.userId} response` },
        };
    },
});
```
